...
withArduino :: Arduino -> (ArduinoChannel -> IO a) -> IO a
withArduino Arduino{device} act =
  ...
  withDetachedKernelDriver devHndl 0  $ do
        ...
        let recv nb to = do
                (bs, status) <- readInterrupt devHndl (endpointAddress endpoint1) nb to
                case status of
                  TimedOut  -> return Nothing
                  Completed -> return $ Just $ concatMap show $ B.unpack bs
        let send str to = do
                (_, status) <- writeInterrupt devHndl (endpointAddress endpoint1) (B.pack (map (fromIntegral . ord) str)) to
                case status of
                  TimedOut  -> return False
                  Completed -> return True
        act $ ArduinoChannel recv send
